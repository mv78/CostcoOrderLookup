...
![Network tab location](https://i.imgur.com/placeholder-network-tab.png)
...
2. Look for a request to `ecom-api.costco.com` — it will appear as `graphql` or similar
3. Click on that request

![Filter by graphql](https://i.imgur.com/placeholder-filter.png)
...
![Copy token from header](https://i.imgur.com/placeholder-copy-token.png)
...
| Token | Lifetime | How it's used |
|-------|----------|---------------|
| `id_token` | ~1 hour | Sent with every API request |
| `refresh_token` | 90 days | Silently obtains a new id_token |

The app tries to silently renew your token using a refresh token (no browser needed). If the renewal endpoint is unreachable (Costco blocks automated access), it will prompt you to re-inject a token from Chrome.
...
└── SETUP.md          # advanced endpoint discovery guide
```
...
- **Credentials** are stored in the OS keyring (Windows Credential Manager / macOS Keychain) — never in plain text
- **Tokens** are cached in `.token_cache.json` which is gitignored
- **HAR files** from Chrome contain sensitive data — never commit them; add them to `.gitignore`
